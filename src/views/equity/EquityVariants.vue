...
import { variantRows, variants, money, number } from './variantData'
...
const selected = computed(() => instruments.value.find(p => p.code === selectedCode.value))
...
const orders = computed(() => demoOrders.value.filter(o => showAll.value || o.account === accountId.value))
  ...
  <main class="variants-app" :class="`version-${variant}`">
    ...
    <div ref="workspace" class="variants-workspace" :class="[`dock-${dock}`,{'ticket-collapsed':collapsed,'is-dragging':dragging}]">
      <section class="positions-pane workspace-panel">
        ...
        <template v-else-if="tab==='orders'"><div class="position-filters dim">当前演示委托 · 不发送至交易系统</div><el-table :data="orders" height="100%" empty-text="暂无模拟委托"><el-table-column prop="account" label="账户" width="100"/><el-table-column prop="name" label="标的名称"/><el-table-column label="方向" width="70"><template #default="{row}"><span :class="row.side==='buy'?'up':'down'">{{ row.side==='buy'?'买入':'卖出' }}</span></template></el-table-column><el-table-column label="类型" width="70"><template #default="{row}">{{ row.type==='limit'?'限价':'市价' }}</template></el-table-column><el-table-column label="价格" width="85"><template #default="{row}">{{ row.price === null ? '市场价格' : money(row.price) }}</template></el-table-column><el-table-column prop="quantity" label="数量"/><el-table-column prop="status" label="状态"/><el-table-column label="操作" width="65"><template #default="{row}"><el-button link :disabled="row.status==='已撤销'" @click="row.status='已撤销'">撤销</el-button></template></el-table-column></el-table></template>
      ...
      <section class="trade-dock" :class="{floating:dock==='floating','assets-collapsed':assetsCollapsed}" :style="floatStyle">
        ...
        <section v-show="!collapsed" class="ticket-pane workspace-panel">
          ...
          <OrderTicket :instruments="instruments" :accounts="accounts" :symbol="selected" :account="account" :quote="quote" @account-select="id=>accountId=id" @select="code=>selectedCode=code" @order="acceptOrder" />
        </section>
